...
function genericPage(name) {
  ...
  const all = published();
  const rel = relatedMap[normalized] || [];
  let list = normalized === "Indeks PRO" ? all : all.filter(a => a.category === normalized);
  if (list.length < 5) list = list.concat(all.filter(a => rel.includes(a.category) && !list.includes(a)).slice(0, 5 - list.length));
...
window.deleteOpinion = id => {
  ...
  if (!t) return;
  closeShareMenus();
  confirmModal({ danger: true, title: "Hapus opini?", message: `"${(t.title || "").slice(0, 60)}" akan dihapus permanen dari halaman Opini.`, confirmLabel: "Ya, hapus", onConfirm: async () => { await saveOpinions(items.filter(x => x.id !== Number(id))); toast("Opini dihapus."); route(); } });
};
...
window.deleteForumThread = id => {
  ...
  if (!t) return;
  closeShareMenus();
  confirmModal({ danger: true, title: "Hapus diskusi?", message: `"${(t.title || "").slice(0, 60)}" akan dihapus permanen dari forum.`, confirmLabel: "Ya, hapus", onConfirm: async () => { await saveForum(threads.filter(x => x.id !== Number(id))); localStorage.removeItem(`kodya-vote-${id}`); toast("Diskusi dihapus."); route(); } });
};
window.deleteForumComment = (id, ci) => {
  ...
  confirmModal({ danger: true, title: "Hapus komentar?", message: `Komentar dari ${(c.author || "").slice(0, 40)} akan dihapus permanen.`, confirmLabel: "Ya, hapus", onConfirm: async () => { t.comments.splice(ci, 1); await saveForum(threads); toast("Komentar dihapus."); route(); } });
};
...
function articlePage(id) { const a = articles().find(x => x.id === Number(id)) || published()[0]; if (!a) return publicChrome(`<main class="page"><div class="shell"><div class="empty" style="padding:80px 0">Artikel tidak ditemukan.</div></div></main>`); const rel = published().filter(x => x.id !== a.id).slice(0, 3); const role = a.category === "Podcast" ? "Produser Podcast Kodya.id" : a.category === "Video" ? "Produser Video Kodya.id" : "Reporter Kodya.id"; return publicChrome(`<main class="page"><div class="shell article-page"><header class="article-heading"><span class="tag">${esc(a.category)}</span><h1>${esc(a.title)}</h1><p class="dek">${esc(a.excerpt)}</p><div class="article-author"><img class="avatar" src="https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&w=100&q=70" alt=""><span><strong>${esc(a.author)}</strong><br><span class="meta">${role} · ${esc(a.date)} · Diperbarui 10:42 WIB</span></span></div></header><img class="article-hero" src="${a.image || IMAGE.city}" alt=""><div class="caption">Ilustrasi: Kodya.id / Unsplash</div>${a.category === "Podcast" && a.media ? `<div class="article-media"><audio controls preload="none" src="${esc(a.media)}"></audio></div>` : a.category === "Video" && a.media ? `<div class="article-media"><video controls preload="metadata" poster="${esc(a.image || IMAGE.city)}" src="${esc(a.media)}"></video></div>` : ""}<div class="article-layout"><article class="article-body"><p>JAKARTA — ${esc(a.excerpt)} Perubahan yang berlangsung hari ini membentuk ruang baru bagi Indonesia untuk mengambil peran yang lebih berarti di tengah dinamika global.</p><p>Sejumlah indikator memperlihatkan arah yang beragam. Di satu sisi, peluang pertumbuhan tetap terbuka melalui inovasi dan penguatan kapasitas domestik. Namun di sisi lain, pelaku ekonomi perlu bersiap menghadapi ketidakpastian eksternal yang belum sepenuhnya mereda.</p><h2>Menjaga ruang tumbuh</h2><p>Perumusan kebijakan yang konsisten menjadi penopang penting. Dalam konteks ini, kolaborasi pemerintah, dunia usaha, dan masyarakat sipil tidak cukup hanya berorientasi pada hasil jangka pendek. Kejelasan arah serta keberanian mengeksekusi agenda menjadi pembeda.</p><blockquote>“Yang dibutuhkan bukan hanya optimisme, melainkan kerja yang disiplin serta kemampuan membaca perubahan.”</blockquote><p>Para pengamat mengingatkan bahwa data perlu ditempatkan sebagai dasar percakapan publik. Dengan begitu, setiap keputusan dapat diuji dan setiap kemajuan dapat diukur secara terbuka.</p><h2>Perspektif yang lebih luas</h2><p>Dalam beberapa waktu ke depan, perhatian pasar akan tertuju pada perkembangan indikator ekonomi, kebijakan bank sentral, serta momentum investasi baru. Kodya.id akan terus mengikuti setiap perkembangan dan menyajikan konteks yang diperlukan pembaca.</p><div class="section"><div class="section-head"><h2>Artikel Terkait</h2></div><div class="editorial-grid">${rel.map(storyCard).join("")}</div></div></article><aside><section class="article-snapshot"><h4>MARKET SNAPSHOT</h4><div><span>IHSG</span><strong class="up">6.319 ▲</strong></div><div><span>USD/IDR</span><strong class="down">18.027 ▼</strong></div><div><span>Gold</span><strong class="up">4.080 ▲</strong></div></section><div style="margin-top:30px">${sidebars(published())}</div></aside></div></div></main>`, a.category); }
...
async function route() {
  ...
  if (root === "admin") {
    ...
    else if (parts[1] === "new-podcast") app.innerHTML = articleEditor(null, "Podcast");
    else if (parts[1] === "new-video") app.innerHTML = articleEditor(null, "Video");
    else if (parts[1] === "edit") app.innerHTML = articleEditor(parts[2]);
    else if (["media", "categories", "authors", "analytics", "settings"].includes(parts[1])) app.innerHTML = adminUtility(parts[1]);
...
window.addCategory = async e => { e.preventDefault(); const name = new FormData(e.target).get("name").trim(); const cats = categoriesList(); if (!name) return toast("Nama kategori wajib diisi."); if (cats.includes(name)) return toast("Kategori sudah ada."); cats.push(name); await saveCategories(cats); toast("Kategori ditambahkan."); route(); };
window.removeCategory = async name => { const decoded = decodeURIComponent(name); await saveCategories(categoriesList().filter(c => c !== decoded)); toast("Kategori dihapus."); route(); };
window.updateAuthor = async e => { e.preventDefault(); const name = new FormData(e.target).get("name").trim(); if (!name) return; localStorage.setItem("kodya-author", name); await saveArticles(articles().map(a => ({ ...a, author: name }))); toast("Nama penulis diperbarui di semua artikel."); route(); };
window.changePassword = async e => { e.preventDefault(); const f = new FormData(e.target); const msg = document.getElementById("pw-msg"); const btn = e.submitter; if (btn) btn.disabled = true; try { const d = await apiSend("/api/change-password", "POST", { current: f.get("current"), next: f.get("next") }); if (d.ok) { toast("Password berhasil diubah."); route(); } else if (msg) msg.textContent = d.error || "Gagal mengubah password."; } catch { if (msg) msg.textContent = "Tidak dapat terhubung ke server."; } finally { if (btn) btn.disabled = false; } };
window.addMedia = async e => { e.preventDefault(); const url = new FormData(e.target).get("url").trim(); if (!/^https?:\/\//.test(url)) return toast("URL tidak valid."); await saveMedia([...new Set([url, ...mediaList()])]); toast("Gambar ditambahkan ke media."); route(); };
...
window.login = async e => { e.preventDefault(); const f = new FormData(e.target); const btn = document.getElementById("login-btn"); const err = document.getElementById("login-error"); if (err) err.textContent = ""; if (btn) { btn.disabled = true; btn.textContent = "Memeriksa..."; } try {
  ...
  if (err) err.textContent = d.error || "Email atau password salah.";
...
window.deleteArticle = id => { confirmModal({ danger: true, title: "Hapus artikel?", message: "Artikel ini akan dihapus permanen. Tindakan ini tidak dapat dibatalkan.", confirmLabel: "Ya, hapus", onConfirm: async () => { await saveArticles(articles().filter(a=>a.id!==id)); toast("Artikel dihapus."); route(); } }); };
window.saveArticle = async (e,id) => { e.preventDefault(); const f = new FormData(e.target); const action = e.submitter?.value; const status = action === "publish" ? "Published" : action === "draft" ? "Draft" : f.get("status") || "Draft"; const item = { id: id || Date.now(), title:f.get("title"), excerpt:f.get("excerpt"), content:f.get("content"), category:f.get("category"), author:f.get("author") || session()?.name || siteAuthor(), image:f.get("image") || IMAGE.city, media: f.get("media") || "", status, featured:f.get("featured") === "on", breaking:f.get("breaking") === "on", views: id ? (articles().find(x=>x.id===id)?.views || "0") : "0", date: dateNow() }; const all = articles(); await saveArticles(id ? all.map(a=>a.id===id?item:a) : [item,...all]); toast(id ? "Perubahan disimpan." : (status === "Published" ? "Berita berhasil dipublikasikan." : "Berita disimpan sebagai draft.")); go("#/admin/articles"); };
