...
function listItem(a) { return `<article class="list-item"><a href="#/artikel/${a.id}"><img src="${a.image || IMAGE.city}" alt="" loading="lazy"></a><div><span class="tag">${esc(a.category)}</span><a href="#/artikel/${a.id}"><h3>${esc(a.title)}</h3></a><p>${esc(a.excerpt)}</p><span class="meta">${esc(a.author)} · ${esc(a.date)}</span></div></article>`; }
...
// ---------- Pemutar audio kustom (podcast & audio artikel) ----------
let __ppAudio = null; // satu elemen audio global, biar cuma satu pemutar yang nyala
const PP_PLAY_ICON = '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8.3 5.4v13.2c0 .86.94 1.4 1.68.96l10.28-6.6a1.12 1.12 0 0 0 0-1.92L9.98 4.44A1.12 1.12 0 0 0 8.3 5.4Z"/></svg>';
const PP_PAUSE_ICON = '<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="6.6" y="5" width="3.7" height="14" rx="1.5"/><rect x="13.7" y="5" width="3.7" height="14" rx="1.5"/></svg>';
...
function podcastPlayer(src, title, art) {
  return `<div class="podcast-player" data-src="${esc(src)}"><div class="pp-art" style="background-image:url('${esc(art || IMAGE.city)}')"></div><div class="pp-main"><div class="pp-title">${esc(title)}</div><div class="pp-controls"><button type="button" class="pp-play" onclick="ppToggle(this)" aria-label="Putar">${PP_PLAY_ICON}</button><div class="pp-bar" onclick="ppSeek(event, this)"><div class="pp-fill"></div></div><span class="pp-time">0:00</span></div></div></div>`;
}
// Pemutar "Dengarkan artikel ini" — untuk pembaca yang lebih suka mendengar daripada membaca
function articleVoicePlayer(a) {
  return `<div class="podcast-player article-audio" data-src="${esc(a.audio)}"><div class="pp-art" style="background-image:url('${esc(a.image || IMAGE.city)}')"></div><div class="pp-main"><div class="article-audio-head"><span class="article-audio-eq"><i></i><i></i><i></i></span><span class="article-audio-label">Dengarkan artikel ini</span><span class="article-audio-meta">versi audio</span></div><div class="pp-title article-audio-title">${esc(a.title)}</div><div class="pp-controls"><button type="button" class="pp-play" onclick="ppToggle(this)" aria-label="Putar audio artikel">${PP_PLAY_ICON}</button><div class="pp-bar" onclick="ppSeek(event, this)"><div class="pp-fill"></div></div><span class="pp-time">0:00</span></div></div></div>`;
}
window.ppToggle = btn => {
  ...
  document.querySelectorAll(".podcast-player.playing").forEach(p => {
    p.classList.remove("playing"); p.querySelector(".pp-play").innerHTML = PP_PLAY_ICON; p.querySelector(".pp-fill").style.width = "0%"; p.querySelector(".pp-time").textContent = "0:00";
  });
  if (audio.src !== player.dataset.src) audio.src = player.dataset.src;
  player.classList.add("playing");
  btn.innerHTML = PP_PAUSE_ICON;
  ...
  audio.onplay = () => { player.classList.add("playing"); btn.innerHTML = PP_PAUSE_ICON; };
  audio.onpause = () => { player.classList.remove("playing"); btn.innerHTML = PP_PLAY_ICON; };
  audio.onended = () => { player.classList.remove("playing"); btn.innerHTML = PP_PLAY_ICON; fill.style.width = "0%"; time.textContent = "0:00"; };
  audio.play().catch(() => { player.classList.remove("playing"); btn.innerHTML = PP_PLAY_ICON; toast("Gagal memutar audio."); });
};
window.ppSeek = (e, bar) => {
...
function videoPage() {
  ...
  return publicChrome(`<main class="page"><div class="shell"><header class="category-title"><span class="tag">KODYA.TV</span><h1>Video</h1><p>Cerita, wawancara, dan penjelasan visual pilihan redaksi.</p></header>
    ...
    <section class="section"><div class="section-head"><h2>Semua Video</h2></div><div class="video-grid">${vids.map(v => `<article class="video-card"><div class="video-thumb" data-media="${esc(v.media || "")}" onclick="playVideo(${v.id})"><img src="${v.image}" alt="" loading="lazy"><span class="media-play">▶</span><span class="media-duration">···</span></div><h3>${esc(v.title)}</h3><p class="meta">${esc(v.date)}</p></article>`).join("")}</div></section>
  </div></main>`, "Video");
}
...
function formatArticleBody(content) {
  if (!content) return "";
  // Content sudah berisi HTML dari editor (contenteditable), render langsung.
  return content;
}
function articlePage(id) { const a = articles().find(x => x.id === Number(id)) || published()[0]; if (!a) return publicChrome(`<main class="page"><div class="shell"><div class="empty" style="padding:80px 0">Artikel tidak ditemukan.</div></div></main>`); const rel = published().filter(x => x.id !== a.id).slice(0, 5); return publicChrome(`<main class="page"><div class="shell article-page"><div class="article-layout"><div class="article-main"><header class="article-heading"><span class="tag">${esc(a.category)}</span><h1>${esc(a.title)}</h1><p class="dek">${esc(a.excerpt)}</p><div class="article-author"><img class="avatar" src="${esc(a.avatar || DEFAULT_AVATAR)}" alt=""><span><strong>${esc(a.author)}</strong><br><span class="meta">${esc(a.date)}</span></span></div></header>${a.category === "Podcast" && a.media ? `<div class="article-media">${podcastPlayer(a.media, a.title, a.image)}</div>` : a.category === "Video" && a.media ? `<div class="article-media"><video controls preload="metadata" poster="${esc(a.image || IMAGE.city)}" src="${esc(a.media)}"></video></div>` : ""}<div class="article-hero"><img src="${a.image || IMAGE.city}" alt=""></div>${a.audio && a.category !== "Podcast" && a.category !== "Video" ? articleVoicePlayer(a) : ""}<article class="article-body">${a.content ? formatArticleBody(a.content) : `<p>${esc(a.excerpt)}</p>`}</article>${a.pullQuote ? `<aside class="article-pullquote"><blockquote>${esc(a.pullQuote)}</blockquote><cite>${esc(a.pullQuoteSource)}</cite></aside>` : ""}<div class="section"><div class="section-head"><h2>Artikel Terkait</h2></div><div class="editorial-grid">${rel.map(storyCard).join("")}</div></div></div><aside class="article-sidebar"><section class="article-snapshot"><h4>MARKET SNAPSHOT</h4><div><span>IHSG</span><strong class="up">6.319 \u25B2</strong></div><div><span>USD/IDR</span><strong class="down">18.027 \u25BC</strong></div><div><span>Gold</span><strong class="up">4.080 \u25B2</strong></div></section><div class="sidebar-divider"></div><h4 class="sidebar-label">ARTIKEL TERPOPULER</h4>${published().slice(0, 5).map((x, i) => `<a class="sidebar-popular" href="#/artikel/${x.id}"><span class="sidebar-popular-no">${i + 1}</span><div><strong>${esc(x.title)}</strong><span class="meta">${esc(x.category)}</span></div></a>`).join("")}</aside></div></div></main>`, a.category); }
...
function togglePassword(btn) { const wrap = btn.closest(".pw-wrap"); const input = wrap.querySelector("input"); const show = input.type === "password"; input.type = show ? "text" : "password"; btn.innerHTML = show ? '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94"/><path d="M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19"/><line x1="1" y1="1" x2="23" y2="23"/></svg>' : '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>'; }
function loginPage() { app.innerHTML = `<main class="login-page"><section class="login-box"><a class="login-back" href="#/">← Kembali ke Beranda</a><a class="brand" href="#/">kodya<em>.id</em></a><h1>Selamat datang.</h1><p>Masuk untuk mengelola newsroom, menulis opini, atau berdiskusi di forum.</p><form onsubmit="login(event)"><label>Email</label><input class="form-control" name="email" type="email" placeholder="email@domain.com" required><label>Password</label><div class="pw-wrap"><input class="form-control" name="password" type="password" placeholder="••••••••" required><button type="button" class="pw-toggle" onclick="togglePassword(this)" tabindex="-1"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg></button></div><p class="login-error" id="login-error"></p><button class="button" id="login-btn">Masuk →</button></form><p class="login-alt">Belum punya akun? <a href="#/register">Daftar akun</a></p></section></main>`; }
function registerPage() { app.innerHTML = `<main class="login-page"><section class="login-box"><a class="login-back" href="#/admin/login">← Kembali ke Login</a><a class="brand" href="#/">kodya<em>.id</em></a><h1>Daftar akun.</h1><p>Buat akun untuk menulis opini, berdiskusi di forum, dan tampil dengan profil Anda.</p><form onsubmit="register(event)"><label>Nama</label><input class="form-control" name="name" placeholder="Nama Anda" required><label>Email</label><input class="form-control" name="email" type="email" placeholder="email@domain.com" required><label>Password</label><div class="pw-wrap"><input class="form-control" name="password" type="password" minlength="6" required><button type="button" class="pw-toggle" onclick="togglePassword(this)" tabindex="-1"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg></button></div><p class="login-error" id="register-error"></p><button class="button" id="register-btn">Daftar →</button></form></section></main>`; }
function profilPage() {
  const me = session() || {};
...
async function trackView(parts) {
  try {
    // Browser yang pernah dipakai login redaksi tidak ikut dihitung (mis. saat cek situs dalam keadaan logout).
    try { if (localStorage.getItem("kodya_staff_browser")) return; } catch { /* abaikan */ }
    const me = session();
    if (me && (me.role === "Super Admin" || me.role === "Penulis")) return; // redaksi tidak dihitung
    ...
    fetch("/api/views", { method: "POST", headers: { "Content-Type": "application/json", ...apiHeaders() }, body: JSON.stringify({ id }) }).catch(() => { });
  } catch { /* abaikan */ }
}
function dashboard() {
  const all = articles();
...
function articleEditor(id = null, presetCategory = null) {
  ...
  const a = existing || { title: "", excerpt: "", category: presetCategory || "Ekonomi", author: siteAuthor(), status: "Draft", featured: false, breaking: false, image: IMAGE.city, content: "", media: "", audio: "", pullQuote: "", pullQuoteSource: "" };
  ...
  const me = session() || {};
  const editorName = me.name || siteAuthor();
  const editorAvatar = me.avatar || DEFAULT_AVATAR;
  const editorRole = me.role === "Super Admin" ? "Admin Kodya.id" : me.role === "Penulis" ? "Penulis Kodya.id" : "Reporter Kodya.id";
  const today = dateNow();
...
window.uploadArticleAudio = async input => {
  const file = input && input.files && input.files[0];
  const msg = document.getElementById("article-audio-msg");
  const nameEl = document.getElementById("article-audio-name");
  if (msg) msg.textContent = "";
  if (!file) return;
  if (nameEl) nameEl.textContent = file.name;
  if (file.size > 25 * 1024 * 1024) { if (msg) msg.textContent = "File terlalu besar (maks 25 MB)."; return; }
  const d = await uploadFile(file);
  if (d.ok) {
    const urlInput = document.querySelector('form.editor [name="audio"]');
    if (urlInput) urlInput.value = d.url;
    if (nameEl) nameEl.textContent = "File terunggah: " + d.url.split("/").pop();
    if (msg) msg.textContent = "Audio artikel terunggah. Simpan artikel untuk menerapkannya di halaman berita.";
    toast("Audio artikel terunggah.");
  } else if (msg) msg.textContent = d.error || "Gagal mengunggah audio.";
};
...
async function generateVideoThumbnails(src) {
  ...
  if (!list || !src) return;
  list.innerHTML = '<span class="panel-subtitle">Memuat…</span>';
  try {
    ...
    list.innerHTML = '<span class="panel-subtitle">Thumbnail otomatis tidak tersedia.</span>';
  }
}
...
window.playVideo = id => { const v = published().find(x => x.id === Number(id)); if (!v) return; const modal = document.createElement("div"); modal.className = "reader-modal"; modal.innerHTML = `<div class="reader-backdrop" onclick="closeReader()"></div><div class="reader-panel media-panel video-modal"><button class="reader-close" onclick="closeReader()" aria-label="Tutup">✕</button><div class="video-frame"><video controls autoplay playsinline poster="${esc(v.image || IMAGE.city)}" src="${v.media || DEMO_VIDEO}"></video></div><div class="video-info"><div class="video-tags"><span class="tag">KODYA.TV</span><span class="video-live">● VIDEO</span></div><h2>${esc(v.title)}</h2><p class="video-desc">${esc(v.excerpt)}</p><div class="video-meta"><span class="video-author"><img class="avatar" src="${esc(v.avatar || DEFAULT_AVATAR)}" alt="">${esc(v.author)}</span><span class="dot">•</span><span>${esc(v.date)}</span></div></div></div>`; document.body.appendChild(modal); document.body.classList.add("reader-open"); };
...
window.openVideoThumbPicker = () => {
  ...
  if (!src) return toast("Unggah file video dulu, baru ambil frame thumbnail.");
  const wrap = document.createElement("div");
  wrap.className = "reader-modal";
  wrap.innerHTML = `<div class="reader-backdrop" onclick="closeVideoThumbPicker()"></div><div class="reader-panel thumb-panel"><button class="reader-close" onclick="closeVideoThumbPicker()" aria-label="Tutup">✕</button><div class="reader-content"><h2>Pilih Frame Thumbnail</h2><div class="thumb-stage"><video id="thumb-video" controls preload="metadata" src="${esc(src)}"></video></div><div class="crop-actions"><button class="button ghost" onclick="closeVideoThumbPicker()">Batal</button><button class="button" id="thumb-capture">Jadikan Thumbnail</button></div><p class="panel-subtitle" id="thumb-msg"></p></div></div>`;
  document.body.appendChild(wrap);
  document.body.classList.add("reader-open");
  wrap.querySelector("#thumb-capture").addEventListener("click", async () => {
    const video = document.getElementById("thumb-video");
    const btn = document.getElementById("thumb-capture");
    const msg = document.getElementById("thumb-msg");
    if (!video || !video.videoWidth) { if (msg) msg.textContent = "Video belum siap — coba tunggu sebentar."; return; }
    ...
    try {
      ...
      const d = await uploadFile(blob);
      if (!d.ok) throw new Error(d.error || "Gagal mengunggah thumbnail.");
      const field = document.querySelector('form.editor [name="image"]');
      if (field) field.value = d.url;
      try { const list = mediaList(); if (!list.includes(d.url)) await saveMedia([...new Set([d.url, ...list])]); } catch { /* opsional */ }
      closeVideoThumbPicker();
      toast("Thumbnail diambil dari video.");
    } catch (err) {
      if (msg) msg.textContent = err.message || "Gagal memproses frame.";
      if (btn) { btn.disabled = false; btn.textContent = "Jadikan Thumbnail"; }
    }
  });
};
window.closeVideoThumbPicker = () => { const m = document.querySelector(".thumb-panel"); if (m) m.closest(".reader-modal").remove(); document.body.classList.remove("reader-open"); };
...
window.saveArticle = async (e,id) => { e.preventDefault(); const f = new FormData(e.target); const editorEl = document.getElementById("content-editor"); const contentVal = editorEl ? editorEl.innerHTML : f.get("content"); const action = e.submitter?.value; const status = action === "publish" ? "Published" : action === "draft" ? "Draft" : f.get("status") || "Draft"; const existing = id ? articles().find(x => x.id === id) : null;
  const item = { id: id || Date.now(), title:f.get("title"), excerpt:f.get("excerpt"), content: contentVal, category:f.get("category"), author: session()?.name || siteAuthor(), avatar: session()?.avatar || "", authorEmail: session()?.email || "", image:f.get("image") || IMAGE.city, media: f.get("media") || "", audio: f.get("audio") || "", pullQuote: f.get("pullQuote") || "", pullQuoteSource: f.get("pullQuoteSource") || "", status, featured:f.get("featured") === "on", breaking:f.get("breaking") === "on", views: existing ? (existing.views || 0) : 0, date: existing?.date || dateNow(), updatedAt: dateNow() + " · " + wibNow() + " WIB" }; const all = articles(); await saveArticles(id ? all.map(a=>a.id===id?item:a) : [item,...all]); toast(id ? "Perubahan disimpan." : (status === "Published" ? "Berita berhasil dipublikasikan." : "Berita disimpan sebagai draft.")); go("#/admin/articles"); };
