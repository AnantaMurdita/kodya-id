...
function articlePage(id) { const a = articles().find(x => x.id === Number(id)) || published()[0]; if (!a) return publicChrome(`<main class="page"><div class="shell"><div class="empty" style="padding:80px 0">Artikel tidak ditemukan.</div></div></main>`); const rel = published().filter(x => x.id !== a.id).slice(0, 5); const role = a.category === "Podcast" ? "Produser Podcast Kodya.id" : a.category === "Video" ? "Produser Video Kodya.id" : "Reporter Kodya.id"; const wordCount = (a.content || "").replace(/<[^>]*>/g, "").split(/\s+/).filter(Boolean).length; const readMin = Math.max(1, Math.ceil(wordCount / 200)); return publicChrome(`<main class="page"><div class="shell article-page"><div class="article-layout"><div class="article-main"><header class="article-heading"><span class="tag">${esc(a.category)}</span><h1>${esc(a.title)}</h1><p class="dek">${esc(a.excerpt)}</p><div class="article-author"><img class="avatar" src="${esc(a.avatar || DEFAULT_AVATAR)}" alt=""><span><strong>${esc(a.author)}</strong><br><span class="meta">${role} \u00b7 ${esc(a.date)}${a.updatedAt ? ` \u00b7 Diperbarui ${esc(a.updatedAt)}` : ""} \u00b7 ${readMin} menit baca</span></span></div></header>${a.category === "Podcast" && a.media ? `<div class="article-media">${podcastPlayer(a.media, a.title, a.image)}</div>` : a.category === "Video" && a.media ? `<div class="article-media"><video controls preload="metadata" poster="${esc(a.image || IMAGE.city)}" src="${esc(a.media)}"></video></div>` : ""}<div class="article-hero"><img src="${a.image || IMAGE.city}" alt=""></div><article class="article-body">${a.content ? formatArticleBody(a.content) : `<p>${esc(a.excerpt)}</p>`}</article>${a.pullQuote ? `<aside class="article-pullquote"><blockquote>${esc(a.pullQuote)}</blockquote><cite>${esc(a.pullQuoteSource)}</cite></aside>` : ""}<div class="section"><div class="section-head"><h2>Artikel Terkait</h2></div><div class="editorial-grid">${rel.map(storyCard).join("")}</div></div></div><aside class="article-sidebar"><section class="article-snapshot"><h4>MARKET SNAPSHOT</h4><div><span>IHSG</span><strong class="up">6.319 \u25B2</strong></div><div><span>USD/IDR</span><strong class="down">18.027 \u25BC</strong></div><div><span>Gold</span><strong class="up">4.080 \u25B2</strong></div></section><div class="sidebar-divider"></div><h4 class="sidebar-label">ARTIKEL TERPOPULER</h4>${published().slice(0, 5).map((x, i) => `<a class="sidebar-popular" href="#/artikel/${x.id}"><span class="sidebar-popular-no">${i + 1}</span><div><strong>${esc(x.title)}</strong><span class="meta">${esc(x.category)}</span></div></a>`).join("")}</aside></div></div></main>`, a.category); }
...
function articleEditor(id = null, presetCategory = null) {
  ...
  const a = existing || { title: "", excerpt: "", category: presetCategory || "Ekonomi", author: siteAuthor(), status: "Draft", featured: false, breaking: false, image: IMAGE.city, content: "", media: "", pullQuote: "", pullQuoteSource: "" };
