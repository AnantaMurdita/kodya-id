...
window.playVideo = id => { const v = published().find(x => x.id === Number(id)); if (!v) return; const modal = document.createElement("div"); modal.className = "reader-modal"; modal.innerHTML = `<div class="reader-backdrop" onclick="closeReader()"></div><div class="reader-panel media-panel video-modal"><button class="reader-close" onclick="closeReader()" aria-label="Tutup">✕</button><div class="video-frame"><video controls autoplay playsinline poster="${esc(v.image || IMAGE.city)}" src="${v.media || DEMO_VIDEO}"></video></div><div class="video-info"><div class="video-tags"><span class="tag">KODYA.TV</span><span class="video-live">● VIDEO</span></div><h2>${esc(v.title)}</h2><p class="video-desc">${esc(v.excerpt)}</p><div class="video-meta"><span class="video-author"><img class="avatar" src="${esc(v.avatar || DEFAULT_AVATAR)}" alt="">${esc(v.author)}</span><span class="dot">•</span><span>${esc(v.date)}</span><span class="dot">•</span><span>${esc(v.views)} views</span></div></div></div>`; document.body.appendChild(modal); document.body.classList.add("reader-open"); };
